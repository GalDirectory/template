...
class FilterModal extends Component {
  constructor(props) {
    ...
    let lookingResults = lookData.map(item => {
      ...
      if (Boolean(interested)) item.selected = interested.includes(item.title);
    ...
    let showResults = showData.map(item => {
      ...
      if (Boolean(showMe)) item.selected = showMe.includes(item.title);
  ...
  donePress = () => {
    ...
    if (getLooking.length > 0)
      filterData.interested = getLooking.map(function (o) {
        return o.title;
      });
    ...
    if (getShowMe.length > 0)
      filterData.showMe = getShowMe.map(function (o) {
        return o.title;
      });
      ...
      <View style={[styles.container]}>
        <View
          ...
          <View
            ...
            <Text
              style={[styles.titleText, {color: theme.subPrimaryColor}]}
            ...
            <View style={{paddingBottom: 20}}>
              <View
                ...
                <Slider
                  ...
                  thumbStyle={[
                    styles.sliderTrack,
                    {shadowColor: theme.pinkColor},
                  ]}
                  thumbTintColor={theme.pinkColor}
                  thumbTouchSize={{width: 32, height: 20}}
                  onValueChange={selectedDistance =>
                    this.setState({
                      selectedDistance: Math.round(selectedDistance),
                    })
                  }
                />
              </View>
                  ...
                  <View style={[styles.itemView]}>
                    ...
                    <View style={[styles.rightRowView]}>
                      <Icon
                        ...
                        size={22}
                        color={theme.subPrimaryColor}
                        style={{fontSize: 22, color: theme.subPrimaryColor}}
                      />
                    </View>
                  </View>
                </TouchableFeedback>
              ...
              <View
                ...
                <Slider
                  ...
                  thumbTintColor={theme.pinkColor}
                  thumbTouchSize={{width: 32, height: 20}}
                  onValueChange={selectedAge =>
                    this.setState({selectedAge: Math.round(selectedAge)})
                  }
                />
              </View>
            </View>
